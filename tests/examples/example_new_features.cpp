...
TEST_F(NewFeaturesExampleTest, FormulaFeatures) {
    ...
    // 设置基础数据
    sheet->setCellValue(1, 1, 100.0);   // A1 = 100
    sheet->setCellValue(1, 2, 200.0);   // B1 = 200
    sheet->setCellValue(1, 3, 300.0);   // C1 = 300
    
    // 设置公式
    bool success = sheet->setCellFormula(2, 1, "SUM(A1:C1)");  // A2 = SUM(A1:C1)
    EXPECT_TRUE(success);
    
    success = sheet->setCellFormula(2, 2, "AVERAGE(A1:C1)");   // B2 = AVERAGE(A1:C1)
    EXPECT_TRUE(success);
    
    success = sheet->setCellFormula(2, 3, "MAX(A1:C1)");       // C2 = MAX(A1:C1)
    EXPECT_TRUE(success);
...
TEST_F(NewFeaturesExampleTest, MergedCellsFeatures) {
    ...
    // 设置标题
    sheet->setCellValue(1, 1, "销售报表");
    
    // 合并标题单元格 A1:D1
    bool success = sheet->mergeCells(1, 1, 1, 4);
    EXPECT_TRUE(success);
    
    // 设置表头
    sheet->setCellValue(2, 1, "产品");
    sheet->setCellValue(2, 2, "Q1");
    sheet->setCellValue(2, 3, "Q2");
    sheet->setCellValue(2, 4, "合计");
    
    // 设置数据
    sheet->setCellValue(3, 1, "产品A");
    sheet->setCellValue(3, 2, 1000.0);
    sheet->setCellValue(3, 3, 1200.0);
    
    sheet->setCellValue(4, 1, "产品B");
    sheet->setCellValue(4, 2, 800.0);
    sheet->setCellValue(4, 3, 900.0);
    
    // 合并产品列的小计行 A5:B5
    sheet->setCellValue(5, 1, "小计");
    success = sheet->mergeCells(5, 1, 5, 2);
    EXPECT_TRUE(success);
    
    // 检查合并状态
    bool isMerged = sheet->isCellMerged(1, 1);
    EXPECT_TRUE(isMerged);
    
    isMerged = sheet->isCellMerged(1, 3);  // A1:D1范围内的C1也应该被合并
    EXPECT_TRUE(isMerged);
    
    // 获取合并区域
    auto mergeRegion = sheet->getMergeRegion(1, 1);
    ...
    // 拆分一个合并区域
    success = sheet->unmergeCells(5, 1);
    EXPECT_TRUE(success);
    
    // 验证拆分后的状态
    isMerged = sheet->isCellMerged(5, 1);
...
TEST_F(NewFeaturesExampleTest, NumberFormatFeatures) {
    ...
    // 设置标题
    sheet->setCellValue(1, 1, "格式类型");
    sheet->setCellValue(1, 2, "原始值");
    sheet->setCellValue(1, 3, "格式化后");
    
    double testValue = 1234.5678;
    
    // 数字格式
    sheet->setCellValue(2, 1, "数字格式");
    sheet->setCellValue(2, 2, testValue);
    bool success = sheet->setCellNumberFormat(2, 3, TinaXlsx::TXCell::NumberFormat::Number, 2);
    EXPECT_TRUE(success);
    sheet->setCellValue(2, 3, testValue);
    
    // 货币格式
    sheet->setCellValue(3, 1, "货币格式");
    sheet->setCellValue(3, 2, testValue);
    success = sheet->setCellNumberFormat(3, 3, TinaXlsx::TXCell::NumberFormat::Currency, 2);
    EXPECT_TRUE(success);
    sheet->setCellValue(3, 3, testValue);
    
    // 百分比格式
    double percentValue = 0.1234;
    sheet->setCellValue(4, 1, "百分比格式");
    sheet->setCellValue(4, 2, percentValue);
    success = sheet->setCellNumberFormat(4, 3, TinaXlsx::TXCell::NumberFormat::Percentage, 2);
    EXPECT_TRUE(success);
    sheet->setCellValue(4, 3, percentValue);
    
    // 科学计数法
    double largeValue = 1234567890.0;
    sheet->setCellValue(5, 1, "科学计数法");
    sheet->setCellValue(5, 2, largeValue);
    success = sheet->setCellNumberFormat(5, 3, TinaXlsx::TXCell::NumberFormat::Scientific, 2);
    EXPECT_TRUE(success);
    sheet->setCellValue(5, 3, largeValue);
    
    // 日期格式
    double dateValue = TinaXlsx::TXNumberFormat::getCurrentExcelDate();
    sheet->setCellValue(6, 1, "日期格式");
    sheet->setCellValue(6, 2, dateValue);
    success = sheet->setCellNumberFormat(6, 3, TinaXlsx::TXCell::NumberFormat::Date);
    EXPECT_TRUE(success);
    sheet->setCellValue(6, 3, dateValue);
    
    // 自定义格式
    sheet->setCellValue(7, 1, "自定义格式");
    sheet->setCellValue(7, 2, testValue);
    success = sheet->setCellCustomFormat(7, 3, "#,##0.00 \"元\"");
    EXPECT_TRUE(success);
    sheet->setCellValue(7, 3, testValue);
    
    // 获取格式化后的值
    std::string formattedValue = sheet->getCellFormattedValue(2, 3);
    std::cout << "数字格式化结果: " << formattedValue << std::endl;
    
    formattedValue = sheet->getCellFormattedValue(3, 3);
    std::cout << "货币格式化结果: " << formattedValue << std::endl;
    
    formattedValue = sheet->getCellFormattedValue(4, 3);
    std::cout << "百分比格式化结果: " << formattedValue << std::endl;
    
    // 批量设置格式
    std::vector<std::pair<TinaXlsx::TXCoordinate, TinaXlsx::TXCell::NumberFormat>> formats;
    formats.emplace_back(TinaXlsx::TXCoordinate(8, 1), TinaXlsx::TXCell::NumberFormat::Number);
    formats.emplace_back(TinaXlsx::TXCoordinate(8, 2), TinaXlsx::TXCell::NumberFormat::Currency);
    formats.emplace_back(TinaXlsx::TXCoordinate(8, 3), TinaXlsx::TXCell::NumberFormat::Percentage);
...
TEST_F(NewFeaturesExampleTest, ComprehensiveExample) {
    ...
    // 1. 标题行（合并单元格）
    sheet->setCellValue(1, 1, "2024年度销售业绩报表");
    sheet->mergeCells(1, 1, 1, 6);
    
    // 2. 表头
    sheet->setCellValue(3, 1, "产品");
    sheet->setCellValue(3, 2, "单价");
    sheet->setCellValue(3, 3, "数量");
    sheet->setCellValue(3, 4, "金额");
    sheet->setCellValue(3, 5, "税率");
    sheet->setCellValue(3, 6, "含税金额");
    ...
    for (size_t i = 0; i < products.size(); ++i) {
        ...
        // 产品名称
        sheet->setCellValue(row, 1, products[i]);
        
        // 单价（货币格式）
        sheet->setCellValue(row, 2, prices[i]);
        sheet->setCellNumberFormat(row, 2, TinaXlsx::TXCell::NumberFormat::Currency, 2);
        
        // 数量（数字格式）
        sheet->setCellValue(row, 3, static_cast<double>(quantities[i]));
        sheet->setCellNumberFormat(row, 3, TinaXlsx::TXCell::NumberFormat::Number, 0);
        
        // 金额（公式：单价*数量）
        std::string formula = "B" + std::to_string(row) + "*C" + std::to_string(row);
        sheet->setCellFormula(row, 4, formula);
        sheet->setCellNumberFormat(row, 4, TinaXlsx::TXCell::NumberFormat::Currency, 2);
        
        // 税率（百分比格式）
        sheet->setCellValue(row, 5, 0.13);  // 13%税率
        sheet->setCellNumberFormat(row, 5, TinaXlsx::TXCell::NumberFormat::Percentage, 1);
        
        // 含税金额（公式：金额*(1+税率)）
        formula = "D" + std::to_string(row) + "*(1+E" + std::to_string(row) + ")";
        sheet->setCellFormula(row, 6, formula);
        sheet->setCellNumberFormat(row, 6, TinaXlsx::TXCell::NumberFormat::Currency, 2);
    }
    
    // 4. 合计行
    int totalRow = 4 + static_cast<int>(products.size());
    sheet->setCellValue(totalRow, 1, "合计");
    sheet->mergeCells(totalRow, 1, totalRow, 3);  // 合并"合计"跨3列
    
    // 金额合计（公式：SUM）
    std::string sumFormula = "SUM(D4:D" + std::to_string(totalRow - 1) + ")";
    sheet->setCellFormula(totalRow, 4, sumFormula);
    sheet->setCellNumberFormat(totalRow, 4, TinaXlsx::TXCell::NumberFormat::Currency, 2);
    
    // 含税金额合计
    sumFormula = "SUM(F4:F" + std::to_string(totalRow - 1) + ")";
    sheet->setCellFormula(totalRow, 6, sumFormula);
    sheet->setCellNumberFormat(totalRow, 6, TinaXlsx::TXCell::NumberFormat::Currency, 2);
    ...
    sheet->setCellValue(totalRow + 2, 1, "报表生成日期:");
    sheet->setCellValue(totalRow + 2, 2, currentDate);
    sheet->setCellNumberFormat(totalRow + 2, 2, TinaXlsx::TXCell::NumberFormat::Date);
...
TEST_F(NewFeaturesExampleTest, PerformanceTest) {
    ...
    for (int row = 1; row <= ROWS; ++row) {
        for (int col = 1; col <= COLS; ++col) {
            ...
            values.emplace_back(TinaXlsx::TXCoordinate(row, col), value);
        }
    }
    ...
    for (int col = 1; col <= COLS; ++col) {
        ...
        formats.emplace_back(TinaXlsx::TXCoordinate(1, col), fmt);
    }
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        mergeRegions.emplace_back(startRow, 1, endRow, 2);
    }
