...
TXCell::TXCell() 
    ...
    , number_format_(NumberFormat::General)
    , custom_format_("")
    , formula_("")
    , is_merged_(false)
...
TXCell::TXCell(TXCell&& other) noexcept
    ...
    , type_(other.type_)
    , number_format_(other.number_format_)
    , custom_format_(std::move(other.custom_format_))
...
void TXCell::setCustomFormat(const std::string& format) {
...
void TXCell::setMasterCell(bool is_master) {
    is_master_cell_ = is_master;
}
...
void TXCell::setMasterCellPosition(int row, int col) {
    master_row_ = row;
    master_col_ = col;
}

std::pair<int, int> TXCell::getMasterCellPosition() const {
...
u32 TXCell::getStyleIndex() const {
    return style_index_;
}
...
TXCell& TXCell::operator=(int value) {
    setValue(static_cast<int64_t>(value));
    return *this;
}
...
TXCell::CellValue TXCell::evaluateFormula(const TXSheet* sheet, uint32_t currentRow, uint32_t currentCol) {
    ...
    try {
        auto result = formula_object_->evaluate(sheet, row_t(currentRow), column_t(currentCol));
